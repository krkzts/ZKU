...
uint aux_positions [7]= {8,9,10,11,12,13,14};
for (uint i_aux = 0; i_aux < 7; i_aux++) {
uint i = aux_positions[i_aux];
std::string new_cmp_name = "nodes"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
...
Fr_lt(&expaux[0],&lvar[17],&circuitConstants[31]); // line circom 17
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[18];
// load src
Fr_div(&expaux[2],&lvar[17],&circuitConstants[5]); // line circom 18
Fr_add(&expaux[1],&expaux[2],&circuitConstants[17]); // line circom 18
Fr_sub(&expaux[0],&expaux[1],&circuitConstants[3]); // line circom 18
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[0],&lvar[17],&circuitConstants[5]); // line circom 20
// end load src
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_sub(&expaux[0],&lvar[17],&circuitConstants[3]); // line circom 21
// end load src
Fr_copy(aux_dest,&lvar[((1 * Fr_toInt(&expaux[0])) + 2)]);
}
// run sub component if needed
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[18])) + 0)]].signalStart + 0]);
}
{
PFrElement aux_dest = &lvar[17];
// load src
Fr_add(&expaux[0],&lvar[17],&circuitConstants[5]); // line circom 17
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[17],&circuitConstants[31]); // line circom 17
